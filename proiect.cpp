...
class SpatiuAerian {
	int lungimeTraversata;
	
public:
	void virtual calcIndicePoluare() = 0;
	SpatiuAerian() {
		this->lungimeTraversata = 10000;
	}
	SpatiuAerian(int lungimeTraversata) {
		this->lungimeTraversata = lungimeTraversata;
	}
	int getLungimeTraversata() {
		return this->lungimeTraversata;
	}
	void setLungimeTraversata(int lungimeTraversata) {
		this->lungimeTraversata = lungimeTraversata;
	}
};

class Avion : public SpatiuAerian {
private:
	string model;
...
public:
	...
	void calcIndicePoluare() {
		cout << "Indicele de poluare al acestui avion este de: " << this->nrTurbine / 100.0<<endl;
	}
	Avion() : nrAvioane(nrInitialAvioane++), SpatiuAerian() {
		model = "Boeing 49";
		nrTurbine = 3;
...
class Companii {
	int profitAn;
public:
	void virtual calcProfitTrecut() = 0;
	Companii() {
		this->profitAn = 1.2;
	}
	Companii(int profitAn) {
		this->profitAn = profitAn;
	}
	int getProfitAn() {
		return profitAn;
	}
	void setProfitAn(int profitAn) {
		this->profitAn = profitAn;
	}
};

class CompanieAeriana: public Companii {
private:
	string nume;
	int numarAeronave;
	float valoareNetaM;
...
public:
	...
	void virtual calcProfitTrecut() {
		cout << "Profitul cumulat din trecut pana in ultimul an intreg este de: " << this->valoareNetaM - getProfitAn()<<endl;
	};
...
class AvionVanatoare : public Avion {
private:
	int nrArmament;
	float* turatieTurbine;
public:
	AvionVanatoare() : Avion() {
		this->nrArmament = 10;
		this->turatieTurbine = new float[getNrTurbine()] {4500.99, 3459.2, 6000.3};
	}
	AvionVanatoare(int nrArmament, float* turatieTurbine, string model, int nrTurbine, string* numeTurbine) : Avion(model, nrTurbine, numeTurbine) {
		this->nrArmament = nrArmament;
		this->turatieTurbine = new float[getNrTurbine()];
		this->turatieTurbine = turatieTurbine;
	}
	AvionVanatoare(const AvionVanatoare& AV) : Avion(AV){
		this->nrArmament = AV.nrArmament;
		this->turatieTurbine = new float[getNrTurbine()];
		for (int i = 0; i < getNrTurbine(); i++) {
			this->turatieTurbine[i]= AV.turatieTurbine[i];
		}
	}
	~AvionVanatoare() {
		delete[]this->turatieTurbine;
	}
	int getNrArmament() {
		return this->nrArmament;
	}
	void setNrArmament(int nrArmament) {
		this->nrArmament = nrArmament;
	}
	void afisareAvionVanatoare() {
		cout << "Avionul de vanatoare are un numare de armamente egal cu "<< this->nrArmament<<" si fiecare turbina are o turatie de:";
		for (int i = 0; i < getNrTurbine(); i++) {
			cout << this->turatieTurbine[i] << " ";
		}
		cout << endl;
	}
...
class AeroportPrivat : public Aeroport {
private:
	int pretBilet;
	char* DirectorAeroport;
public:
	AeroportPrivat() : Aeroport("BGY_Bergamo_VIP") {
		this->pretBilet = 300;
		this->DirectorAeroport = new char[strlen("Mikhausa Fujiyama") + 1];
		strcpy_s(this->DirectorAeroport, strlen("Mikhausa Fujiyama") + 1, "Mikhausa Fujiyama");
	}
	AeroportPrivat(int pretBilet, char* DirectorAeroport, string numeAeroport, int nrProduse, float* pretProdus) : Aeroport(numeAeroport, nrProduse, pretProdus) {
		this->pretBilet = pretBilet;
		this->DirectorAeroport = new char[strlen("Nickolau Papadopoulos") + 1];
		strcpy_s(this->DirectorAeroport, strlen("Nickolau Papadopoulos") + 1, "Nickolau Papadopoulos");
	}
	AeroportPrivat(const AeroportPrivat& AP) : Aeroport(AP) {
		this->pretBilet = AP.pretBilet;
		this->DirectorAeroport = new char[strlen(AP.DirectorAeroport) + 1];
		strcpy_s(this->DirectorAeroport, strlen(AP.DirectorAeroport) + 1, AP.DirectorAeroport);
	}
	~AeroportPrivat(){
		delete[]this->DirectorAeroport;
	}
	void afisareAeroportPrivat() {
		cout << "Aeroportul privat are pretul mediu pe bilet de: " << this->pretBilet << " si numele directorului este " << this->DirectorAeroport << endl;
	}
};
...
void main() {
	...
	sa[0] = new Avion("Flying76");
	sa[1] = new Avion();
	sa[2] = new Avion("Flying722");
	sa[3] = new Avion("Boeing555");
	sa[4] = new Avion("Traveler",3,numeTurbine);
	sa[5] = new Avion();
	sa[6] = new Avion("Flying77");
	sa[7] = new Avion();
	sa[8] = new Avion("JaguarFalcon");
	sa[9] = new Avion("Flying76");
	for (int i = 0; i < 10; i++) {
		sa[i]->calcIndicePoluare();
	}

	Companii** co;
	co = new Companii*[10];
	co[0] = new CompanieAeriana();
	co[1] = new CompanieAeriana(400.3);
	co[2] = new CompanieAeriana(210.3);
	co[3] = new CompanieAeriana(100.3);
	co[4] = new CompanieAeriana();
	co[5] = new CompanieAeriana(230.3);
	co[6] = new CompanieAeriana();
	co[7] = new CompanieAeriana(20.3);
	co[8] = new CompanieAeriana();
	co[9] = new CompanieAeriana(10.3);
	for (int i = 0; i < 10; i++) {
		co[i]->calcProfitTrecut();
	}
}
